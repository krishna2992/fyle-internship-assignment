...
class Assignment(db.Model):
    ...
    def get_assignments_by_principle(cls, p):
        assertions.assert_auth(p.principal_id==1 , 'UNAUTHORISED')
    ...
    @classmethod
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal, principle=False):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(grade is not None, 'assignment with empty grade cannot be graded')
        if not principle:
           assertions.assert_valid(assignment.teacher_id==auth_principal.teacher_id, 'FyleError')
        if principle:
